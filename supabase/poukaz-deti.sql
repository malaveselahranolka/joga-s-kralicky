-- =====================================================================
--  Jóga s králíčky — dárkový poukaz na lekci Děti & králíčci
--  Spusť: Supabase → SQL Editor → New query → Run. Bezpečné víckrát.
--  Předpoklad: deti-a-kralici.sql (lessons.druh) a všechno před ním.
--
--  DVA DRUHY POUKAZU (vouchers.druh)
--  * klasik  jeden vstup na lekci Jóga s králíčky (499 Kč)
--  * deti    lekce Děti & králíčci pro zákonného zástupce s jedním
--            dítětem (1 090 Kč, stejně jako vstup na tu lekci)
--
--  Druh zapisují stripe-webhook / stripe-confirm z metadat platby
--  (_shared/poukaz-druh.ts). Staré poukazy dostanou výchozí 'klasik'.
--
--  UPLATNĚNÍ (create_booking_poukazem)
--  Poukaz jde jen na lekci stejného druhu:
--    klasický poukaz na dětskou lekci   → 'deti_lekce'
--    dětský poukaz na klasickou lekci   → 'klasik_lekce'
--  Dětský poukaz založí rezervaci na 2 místa (zástupce + 1 dítě), a proto
--  potřebuje 2 volná místa. Další děti poukaz nepokrývá.
--  Druh se kontroluje DŘÍV, než se poukaz spotřebuje, a pro jistotu ještě
--  jednou přímo v UPDATE, který ho uplatňuje.
-- =====================================================================

-- 1) DRUH POUKAZU ------------------------------------------------------
alter table public.vouchers add column if not exists druh text not null default 'klasik';
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'vouchers_druh_check') then
    alter table public.vouchers add constraint vouchers_druh_check check (druh in ('klasik', 'deti'));
  end if;
end $$;
...
begin
  ...
  select druh into v_druh from public.vouchers where code = c;
  ...
  -- klasický poukaz = 1 místo, dětský = zástupce + 1 dítě = 2 místa
  mist := case when l.druh = 'deti' then 2 else 1 end;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'lesson',     l.title,
      'datetime',   kdy,
      'spots',      case when mist = 2 then 'zástupce + 1 dítě' else '1 místo' end,
